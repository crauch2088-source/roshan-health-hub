...
export function ConsultationPage({visitId}:{visitId:string}){
 ...
 const tabs: { key: string; labelAr: string; labelEn: string }[] = [
  { key: "note", labelAr: "الملاحظات", labelEn: "Notes" },
  { key: "vitals", labelAr: "العلامات الحيوية", labelEn: "Vitals" },
  { key: "labs", labelAr: "المختبر", labelEn: "Laboratory" },
  { key: "prescription", labelAr: "الوصفة", labelEn: "Prescription" },
  { key: "history", labelAr: "السجل السابق", labelEn: "History" },
 ];
 return <div className="container mx-auto max-w-7xl space-y-4 p-4" dir="auto">
   ...
   <div className="overflow-x-auto border-b"><div className="flex min-w-max gap-1">{tabs.map((x)=><button key={x.key} onClick={()=>setTab(x.key)} className={`border-b-2 px-4 py-3 text-sm font-medium ${tab===x.key?"border-primary text-primary":"border-transparent text-muted-foreground"}`}>{lang==="ar"?x.labelAr:x.labelEn}</button>)}</div></div>
