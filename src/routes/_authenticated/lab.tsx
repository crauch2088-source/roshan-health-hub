...
function LabPageInner() {
  ...
  const orders = useRows<Row[]>(["lab-orders", status], () => {
    let q = supabase.from("lab_orders")
      .select("id,status,created_at,ordered_at,priority,visit_id,patients(id,full_name,mrn,phone),users!lab_orders_ordered_by_fkey(full_name),lab_order_items(id,status,price,lab_tests(id,name,name_ar))")
  ...
  const rows = all.filter((o) => {
    ...
    return `${s(p,"full_name")} ${s(p,"mrn")} ${s(p,"phone")} ${s(o,"id")} ${s(o,"visit_id")} ${tests}`.toLowerCase().includes(term);
  });
