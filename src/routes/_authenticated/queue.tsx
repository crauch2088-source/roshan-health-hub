...
function VisitsPage() {
   ...
   *
   * IMPORTANT:
   * queue_number is now independent from visit_number.
   */
  const visits = useRows(["visits", date], () =>
    supabase
      .from("visits")
      .select(
        "id, visit_number, queue_number, visit_date, status, visit_type, consultation_fee, patients!visits_patient_id_fkey(id, full_name, mrn), departments!visits_department_id_fkey(name, name_ar), users!visits_doctor_id_fkey(full_name)",
      )
      .eq("visit_date", date)
      .is("deleted_at", null)
   ...
   * Create visit.
   *
   * Queue number is generated by PostgreSQL:
   *
   * Q001
   * Q002
   * Q003
   *
   * independently of the medical visit_number.
   */
  const create = useSave(
    async () => {
      if (!form.patient_id) {
        throw new Error(`${t("patient")} is required.`);
      }

      /*
       * Ask PostgreSQL for the next queue number for this day.
       *
       * This is concurrency-safe and prevents duplicate queue numbers.
       */
      const { data: queueNumber, error: queueError } =
        await supabase.rpc("next_queue_number", {
          _visit_date: date,
        });
      ...
      if (
        ...
        throw new Error(
          "Invalid queue number generated by the database.",
        );
      }
       ...
       * IMPORTANT:
       *
       * queue_number = patient's position in today's queue.
       *
       * visit_number is intentionally NOT generated here.
       * It remains a separate clinical record identifier.
       */
   ...
   * Soft-delete visit.
   *
   * The database trigger will prevent deletion of a locked visit
   * unless the current user is Super Admin.
   */
  const deleteVisit = useSave(
    async (visitId: string) => {
      ...
      /*
       * If a queue ticket exists, cancel it as well.
       */
      ...
      if (queueError) {
        /*
         * Do not fail the whole deletion if the queue ticket
         * does not exist.
         */
        console.warn(
          "Unable to update queue ticket:",
          queueError.message,
        );
      }
