...
function VisitsPage() {
  ...
  const create = useSave(
    async () => {
      ...
      /*
       * The queue number is generated inside PostgreSQL.
       *
       * This avoids Math.random(), which could generate duplicate numbers.
       */
      const { data: queueNumber, error: queueError } =
        await supabase.rpc("next_visit_number", {
          _visit_date: date,
        });
      ...
      if (!Number.isInteger(numericQueue) || numericQueue <= 0) {
        throw new Error("Invalid visit number generated by the database.");
      }
  ...
  const deleteVisit = useSave(
    async (visitId: string) => {
      if (!can("visits.delete")) {
        throw new Error("You do not have permission to delete visits.");
      }
