...
function CertificatesPageInner(){
 ...
 return <div className="space-y-4">
  ...
  <Dialog open={open} onOpenChange={setOpen}><DialogContent className="max-h-[90vh] overflow-y-auto"><DialogHeader><DialogTitle>{lang==="ar"?"إصدار شهادة طبية":"Issue medical certificate"}</DialogTitle></DialogHeader><div className="grid gap-4">
   ...
   <Field label={`${lang==="ar"?"المريض":"Patient"} *`}><Select value={form.patient_id} onValueChange={v=>setForm({...form,patient_id:v})}><SelectTrigger><SelectValue placeholder={lang==="ar"?"اختر المريض":"Select patient"}/></SelectTrigger><SelectContent className="max-h-72">{((patients.data??[]) as Row[]).map(p=><SelectItem key={s(p,"id")} value={s(p,"id")}>{s(p,"full_name")} — {s(p,"mrn")||s(p,"phone")}</SelectItem>)}</SelectContent></Select></Field>
